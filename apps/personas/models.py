...
class PersonaReportada(models.Model):
    # Identificación
    id_caso = models.CharField("ID Caso", max_length=10, unique=True, blank=True, editable=False)
    ...
    nacionalidad_cedula = models.CharField(
        "Tipo cédula", max_length=2, blank=True, choices=NacionalidadCedula.choices
    )
    nombre_completo = models.CharField("Nombre completo", max_length=200)
    alias_o_apodos = models.CharField(
        "Alias / Apodos", max_length=200, blank=True, help_text="Separados por coma"
    )
    edad_aproximada = models.IntegerField("Edad aproximada", null=True, blank=True)
    sexo = models.CharField("Sexo", max_length=15, blank=True, choices=Sexo.choices)
    tipo_sangre = models.CharField(
        "Tipo de sangre", max_length=3, blank=True, choices=TipoSangre.choices
    )

    # Ubicación
    estado_ultima_ubicacion = models.CharField(
        "Estado (última ubicación)",
        max_length=30,
        blank=True,
        choices=EstadoVenezolano.choices,
    )
    detalle_ultima_ubicacion = models.CharField(
        "Detalle ubicación",
        max_length=200,
        blank=True,
        help_text="Sector, parroquia, referencia",
    )
    fecha_ultimo_contacto = models.DateField("Fecha último contacto", default=datetime.date.today)
